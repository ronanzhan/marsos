...
pub fn load_apps() {
    ...
    for i in 0..num_app {
        let base_address_i = get_base_i(i);
        (base_address_i..base_address_i + APP_SIZE_LIMIT).for_each(|addr| unsafe {
            (addr as
        ...
        let dst = unsafe {
            core::slice::from_raw_parts_mut(base_address_i as *mut u8, src.len())
        };
